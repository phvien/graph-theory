...
void traversal(Graph* G, int x){
	if(mark[x]==1)	// Neu dinh x duoc danh dau thi bo qua
		return;	
		
	printf("%d\n", x);	// Neu chua duoc danh dau thi in x ra va danh dau x=1;
	mark[x]=1;
	List list = neighbors(G, x);	// Lay dinh ke cua x cho vao danh sach
	int j;
	for(j=1; j<=list.size; j++){	// Tuong tu depth_first_search
