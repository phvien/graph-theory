...
void breath_first_search(Graph* G) {     
	Queue frontier;     
	int mark[MAX_VERTICES];     
	make_null_queue(&frontier); /* Kh?i t?o mark, chua d?nh nào du?c xét */     
	int j;     
	for (j = 1; j <= G->n; j++)         
		mark[j] = 0; /* Ðua 1 vào frontier */     
	push(&frontier, 1);     
	mark[1] = 1; /* Vòng l?p chính dùng d? duy?t */     
	while (!empty(&frontier)) { /* L?y ph?n t? d?u tiên trong frontier ra */         
